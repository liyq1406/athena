...
public class TransTimeService {
	 ...
	 */
	protected List<Ticxxsj> gongzrList ;
	/**
	 *  单个卸货站台对应的 所有承运商的集合 
	 */
	protected List<CkxYunssjMb> YunssjMbList;
	/**
	 * 用于传递 用户中心+卸货站台
	 */
	protected Ticxxsj t ; 
	 ...
	 * @param baseDao 数据库连接
	 */
	public TransTimeService(AbstractIBatisDao baseDao,Ticxxsj t,List<CkxYunssjMb> YunssjMbList,List<Ticxxsj> gongzrList){
		this.baseDao=baseDao;
		this.YunssjMbList = YunssjMbList;
		this.t = t;
	...
	private void compTransTime(String creator){
		...
		//循环未来几日剔除休息时间  的日期的 集合
		for (Ticxxsj ticxxsj : gongzrList) {//卸货站台的工作日历		
			ticxxsj.setUsercenter(t.getUsercenter());	//用户中心
			ticxxsj.setChanxck(t.getChanxck());			//生产线编号
			List<Ticxxsj> sjList = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ts_ckx.queryTicxxsj", ticxxsj);//按照用户中心+卸货站台编组+工作日获得当天的工作时间
			j++;
			...
			//循环卸货站台所对应的承运商
			for (CkxYunssjMb bean : YunssjMbList) {
				//根据承运商找到单个承运商的所有趟次
				List<CkxYunssjMb> listGcbhAllXuh = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("transTime.queryCkxYunssjMb",bean);
				for (CkxYunssjMb ckxYunssjMb : listGcbhAllXuh) {
					...
					mapYunssk.put("xuh",getXuh(ckxYunssjMb.getXuh(), ticxxsj.getGongzr().toString()));
					// 获取具体的到货时间
					Date daohsjTime  =  getShijByTime( ticxxsj.getGongzr().toString() , ckxYunssjMb.getDaohsj().intValue());
					//如果上线偏移时间超出上班结束时间，则从此趟次开始，后续趟次全部删除
					if (daohsjTime.getTime() == 0){
						break;
					}
					mapYunssk.put("daohsj",	daohsjTime);
					Date facsjTime  =  getShijByTime( ticxxsj.getGongzr().toString() , ckxYunssjMb.getFacsj().intValue());
					// 获取具体的发出时间
