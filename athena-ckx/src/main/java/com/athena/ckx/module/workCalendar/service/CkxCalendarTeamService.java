 ...
 */
@Component
public class CkxCalendarTeamService extends BaseService<CkxCalendarTeam>{
	@Inject
...
//			return new Message("starbigError","i18n.ckx.workCalendar.i18n_workCalendar").getMessage();
		...
		if(!"".equals(usercode)){
			...
			//获取最大编组序号和当前月份
			Object o=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getMaxTeamCode",month);
			String no="01";
			if(o!=null){//本组只有上一条数据
				no=o.toString();
		...
		//获取同一编组同一天的最大序号的值
		Object o=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getMaxXuhCode",bean);
		String no="01";
		...
		if(o!=null){//本组有上一条数据
			...
			//验证时间是否重复
			if(t.getJiezsj().compareTo(bean.getKaissj())>=0&&bean.getTiaozsj().equals(t.getTiaozsj())){
				//上一条的结束时间大于下一条的开始时间
				//msg:"编组内同一天的工作时间有重叠!";
//				return new Message("groupTimeRepeating","i18n.ckx.workCalendar.i18n_workCalendar").getMessage();
		...
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.insertCkxCalendarTeam", bean);//插入 
...
//				return new Message("starbigError","i18n.ckx.workCalendar.i18n_workCalendar").getMessage();
			...
			//当前数据上一条
			Object before=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getTeamBefore",team);
			if(before!=null){
...
//					return "请确定时间是否跨天";
			...
			//当前数据下一条
			Object after=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getTeamAfter",team);
			if(after!=null){
				CkxCalendarTeam t=(CkxCalendarTeam) after;
				
				//下一条未跨天，当前条必须未跨天
				if(t.getTiaozsj()==0&&team.getTiaozsj()!=0){
					return "序号"+team.getXuh()+"的工作时间段不能跨天";
				}
			...
			super.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.updateCkxCalendarTeam", team);
		}
	...
	public String copyTeam(String usercode,String teamcode,LoginUser user){
		...
		List<CkxCalendarTeam> list=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.queryCkxCalendarTeam",team);
		...
		for (CkxCalendarTeam t : list) {
			...
			team.setEditor(user.getUsername());//修改人
			team.setEditTime(DateTimeUtil.getAllCurrTime());//修改时间
			baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.insertCkxCalendarTeam", team);
		}
	...
	public String delTeam(String teamcode,LoginUser user){
		...
		//逻辑删除
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.deleteCkxCalendarTeamLogic",team);
		return new Message("shixSuccess","i18n.ckx.workCalendar.i18n_workCalendar").getMessage();
	}
	/**
	 * 物理删除一行数据(POA)
	 * @param bean
	...
	public String physicalTeam(CkxCalendarTeam bean){
		try {
			baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.deleteCkxCalendarTeam",bean);
	...
	public List<CkxCalendarTeam> getSelectTeamCode(CkxCalendarTeam bean){
		...
		List<String> slist=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.getSelectTeamCode",bean);
