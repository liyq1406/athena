 ...
 */
@Component
public class CkxCalendarVersionService extends BaseService<CkxCalendarVersion>{
	@Inject
	private  CacheManager cacheManager;//缓存
	
	private  Logger logger =Logger.getLogger(CkxCalendarVersionService.class);
	...
	public String edit(ArrayList<CkxCalendarVersion> editList,LoginUser user){
		for (CkxCalendarVersion ckxCalendarVersion : editList) {//循环获取日历版次
			...
			super.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.updateCkxCalendarVersion", ckxCalendarVersion);//更新版次
		}
	...
	public String addVersion(String years,String center,String userCode,LoginUser user){
		...
		String month=Integer.toHexString(Integer.parseInt(s)).toUpperCase();
		
		
		//获取最大版次序号和当前月份
		Object o=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getVersionMaxNo",month);
		String no="01";
		...
		for (String year : strs) {
			...
			baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.addCkxCalendarVersion", map);
		}
	...
	public String copyVersion(String versionCode,String userCode,LoginUser user){
		...
		List<CkxCalendarVersion> list=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.queryCkxCalendarVersion",varsion);
		...
		map.put("banc", banc);
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.copyCkxCalendarVersion", map);
...
//		if(glist!=null&&glist.size()!=0){
		...
		int count = (Integer) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.selectCountByBanc", group);
		if(count!=0){
			//{0}版次的日历正在被使用，无法删除
		...
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.deleteCkxCalendarVersionLogic", version);
		//timingTask();
	...
	public List<CkxCalendarVersion> getVersionNo(CkxCalendarVersion bean){
		...
		List<String> verlist = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.getSelectVersionCode",bean);
	...
	public void timingTask() {
		...
		// // 获取版次号集合
		// List<String> verlist =
		// baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.getVersionCode");
		// List<CkxCalendarVersion> ckxCalendarVersionlist = new
		// ArrayList<CkxCalendarVersion>();
		// CkxCalendarGroup group = new CkxCalendarGroup();
		// for (String versionCode : verlist) {
		// if("NA01KD01".equals(versionCode)||"NA01AX01".equals(versionCode)||"NA01LX01".equals(versionCode)){
		// continue;
		// }
		// //将版次号带入查询关联
		// group.setRilbc(versionCode);
		// int count = (Integer)
		// baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.selectCountByBanc",
		// group);
		// if(count == 0){
		...
		// ckxCalendarVersionlist.add(bean);
		// baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).executeBatch("workCalendar.deleteCkxCalendarVersion",
		// ckxCalendarVersionlist);

		/***************** by liuhuixi 2014-11-18 ***********************/
		/*-- 将所有版次号与仓库产线的版次进行对比，如果当前版次非（NA01KD01，NA01AX01，NA01LX01）创建时间超出15天且没有与仓库产线关联，则物理删除当前版次下的所有日历 --*/
		CkxCalendarVersion bean = new CkxCalendarVersion();
		bean.setStrNA01Banc("'NA01KD01','NA01AX01','NA01LX01'");
		bean.setRiq(Calendar.getInstance().get(Calendar.YEAR) + "");
		// 批量删除日历版次非（NA01KD01，NA01AX01，NA01LX01）创建日期超出15天且没有与仓库产线关联
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute(
				"workCalendar.deleteCkxCalendarVersionByRiAndBanc", bean);
		logger.info("4220接口批量删除日历版次成功");
		// 批量删除工作时间编组创建日期超出15天且没有与仓库产线关联
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute(
				"workCalendar.deleteCkxCalendarTeamByDate");
		logger.info("4220接口批量删除工作时间编组成功");
	}
	...
	public String eidtNianzx(CkxCalendarVersion bean,LoginUser user){	
		//获取将要修改前具体数据
		CkxCalendarVersion version = (CkxCalendarVersion) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getCkxCalendarVersion", bean);
		//获取将要修改后的具体数据(一周最后一天)
		CkxCalendarVersion version1 = (CkxCalendarVersion) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getCkxCalendarVersionBynianzx", bean);
		...
		version.setEditTime(DateTimeUtil.getAllCurrTime());	
		//更新年周序
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.updateNianzxUD", version);
		//更新原年周期需要更新的周序的数据
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.updateZhouqZhouxUD", version);
		...
		if(!bean.getNianzx().equals(version.getNianzxTemp())){			
			if(version1 == null){
				version1 = new CkxCalendarVersion();
				version1.setNianzq(version.getNianzq());
				version1.setNianzx(bean.getNianzx());
				int zhoux = Integer.parseInt(version.getZhoux())+1;				
				version1.setZhoux((zhoux+"").length()>1?zhoux+"":"0"+zhoux);
//				throw new ServiceException(GetMessageByKey.getMessage("wufzdsj",new String[]{bean.getBanc(),bean.getUsercenter(),bean.getNianzx()}));   //"无法找到版次："+bean.getBanc()+",用户中心："+bean.getUsercenter()+",年周序："+bean.getNianzx()+"的数据"
		...
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.updateCkxCalendarVersionZhouxUD", version);
	...
	public int getSpacing(CkxCalendarVersion version,String xnianzx,int spacing){
		...
		if(0 > spacing){
			//取得该年周序的结束日
			 riq = (String) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getMaxRiqByNianzx", version);			
		}else{
			//取得该年周序的开始日
			 riq = (String) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("workCalendar.getMinRiqByNianzx", version);	
		}
	...
	@Transactional
	@SuppressWarnings("unchecked")
	public String mergeTime(String userID){
		//清空合并时间表的数据
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.removeHebTime");
		//获取工作时间关联的数据
		List<CkxCalendarGroup> list = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("workCalendar.queryCkxCalendarGroupHebTime");
		//将数据保存，以供插入到表中
		List<Map<String,Object>> listValue = new ArrayList<Map<String,Object>>();
		//初始化流水号
		Integer liush = 0;
		for (CkxCalendarGroup ckxCalendarGroup : list) {
			Map<String,Object> map = new HashMap<String, Object>();
			map.put("HEBTIMEH", String.format("%08d",liush));
			map.put("USERCENTER", ckxCalendarGroup.getUsercenter());
			map.put("BANC", ckxCalendarGroup.getRilbc());
			map.put("BIANZH", ckxCalendarGroup.getBianzh());
			map.put("WEILBZH", ckxCalendarGroup.getWeilbzh());
			map.put("SHENGXSJ", ckxCalendarGroup.getShengxsj());
			//hanwu 20160105  添加多个未来编组号
			map.put("WEILBZH2", ckxCalendarGroup.getWeilbzh2());
			map.put("SHENGXSJ2", ckxCalendarGroup.getShengxsj2());
			map.put("WEILBZH3", ckxCalendarGroup.getWeilbzh3());
			map.put("SHENGXSJ3", ckxCalendarGroup.getShengxsj3());
			map.put("WEILBZH4", ckxCalendarGroup.getWeilbzh4());
			map.put("SHENGXSJ4", ckxCalendarGroup.getShengxsj4());
			map.put("CREATOR", userID);
			listValue.add(map);
			liush ++;
		}
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).executeBatch("workCalendar.addCalendarHebTime", listValue);
		
		//清空合并时间-详细表的数据
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.removeHebTimeInfo");
		//添加合并时间-详细表的数据
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("workCalendar.addCalendarHebTimeInfo",userID);
		return "success";
	}
}