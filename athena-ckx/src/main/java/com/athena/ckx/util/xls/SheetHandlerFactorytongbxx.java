 ...
 * @date 2012-5-17
 */
public class SheetHandlerFactorytongbxx {
	...
	public static SheetHandler getInstance(Workbook workbook, String sheet,
		...
		if(clazz==null || "null".equals(clazz)){
			...
			handler = new BaseSheetHandlergyxhd(workbook, sheet, table, datasourceId, clazz,keys,dateColumns,dateFormats);
		}else{
	 ...
	 * @param clazz 根据clazz创建出sheet解析器
	 */
	public static SheetHandlertongbxx getInstance(Document document, String sheet,
			String table, String datasourceId, String clazz,String keys,String dateColumns,String dateFormats) {
		SheetHandlertongbxx handler = null;
		if(clazz==null || "null".equals(clazz)){
			//创建默认的sheetHandler解析器处理 
			handler = new XmlSheetHandlertongbxx(document, sheet, table, datasourceId, clazz,keys,dateColumns,dateFormats);
		}else{
			//依据用户的clazz全名 创建用户的sheetHandler对象
...
private static SheetHandlertongbxx createHandler(Document document, String sheet,
	String table, String datasourceId, String clazzStr,String keys,String dateColumns,String dateFormats) {
	SheetHandlertongbxx sh = null;
	try {
		...
		sh = (SheetHandlertongbxx) c.newInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats);
	} catch (Exception e) {
		e.printStackTrace();
	}
