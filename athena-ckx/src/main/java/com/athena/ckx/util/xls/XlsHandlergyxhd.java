...
public class XlsHandlergyxhd {
	...
	public Map<String,Object> getMapshengcx() {
		return mapshengcx;
	}

	public void setMapshengcx(Map<String,Object> mapshengcx) {
		this.mapshengcx = mapshengcx;
	}

	private XlsHandlergyxhd() {
		super();
	}

	public static XlsHandlergyxhd getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,Map<String,Object> mapshengcx,AbstractIBatisDao baseDao){
		XlsHandlergyxhd handler = new XlsHandlergyxhd();
		handler.setIn(in);
		handler.setJsonConfig(jsonConfig);
		handler.setEditor(editor);
		handler.setMap(map);//产线map
		handler.setMapshengcx(mapshengcx);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorygyxhd.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,mapshengcx,baseDao);
				//填充错误信息
