...
public class XlsHandlerjipxx {
	...
	private XlsHandlerjipxx() {
		super();
	}

	public static XlsHandlerjipxx getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,Map<String,Object> pmap,AbstractIBatisDao baseDao){
		XlsHandlerjipxx handler = new XlsHandlerjipxx();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryjipxx.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,pmap,baseDao);
				//填充错误信息
