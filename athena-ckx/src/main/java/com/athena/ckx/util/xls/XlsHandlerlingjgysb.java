 ...
 * @date 2012-5-17
 */
public class XlsHandlerlingjgysb {
	...
	public Map<String, String> getMaplingjgys() {
		return maplingjgys;
	}


	public void setMaplingjgys(Map<String, String> maplingjgys) {
		this.maplingjgys = maplingjgys;
	}


	public Map<String, String> getMapgcy() {
		return mapgcy;
	}


	public void setMapgcy(Map<String, String> mapgcy) {
		this.mapgcy = mapgcy;
	}
	...
	private XlsHandlerlingjgysb() {
		super();
	}

	public static XlsHandlerlingjgysb getInstance(InputStream in,String jsonConfig,String editor,Map<String, String>  maplingj,Map<String,String>  mapgcy,Map<String, String>  mapbaoz,Map<String, String> maplingjgys,AbstractIBatisDao baseDao){
		XlsHandlerlingjgysb handler = new XlsHandlerlingjgysb();
		handler.setIn(in);
		handler.setJsonConfig(jsonConfig);
		handler.setEditor(editor);
		handler.setMaplingj(maplingj);
		handler.setMapgcy(mapgcy);
		handler.setMapbaoz(mapbaoz);
		handler.setMaplingjgys(maplingjgys);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorylingjgysb.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,maplingj,mapgcy,mapbaoz,maplingjgys,baseDao);
				//填充错误信息
