 ...
 * @date 2012-5-17
 */
public class XlsHandlerlingjxhd {
	...
	public void setGongysmap(Map<String, String> gongysmap) {
	...
	private XlsHandlerlingjxhd() {
		super();
	}

	public static XlsHandlerlingjxhd getInstance(InputStream in,String jsonConfig,String editor,Map<String, String>  mapfenpq,Map<String,String>  gongysmap,Map<String, String>  maplingj,Map<String, String>  gyxhdmap,Map<String, String>  xiaohcmap,Map<String, String>  xiaohcbhmap,AbstractIBatisDao baseDao){
		XlsHandlerlingjxhd handler = new XlsHandlerlingjxhd();
		handler.setIn(in);
		handler.setJsonConfig(jsonConfig);
		handler.setEditor(editor);
		handler.setMapfenpq(mapfenpq);//分配区产线map
		handler.setGongysmap(gongysmap);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorylingjxhd.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,mapfenpq,gongysmap,maplingj,gyxhdmap,xiaohcmap,xiaohcbhmap,baseDao);
				//填充错误信息
