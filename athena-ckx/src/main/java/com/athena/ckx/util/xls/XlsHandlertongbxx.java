 ...
 * @date 2012-5-17
 */
public class XlsHandlertongbxx {
	...
	private XlsHandlertongbxx() {
		super();
	}

	public static XlsHandlertongbxx getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,Map<String,Object> pmap,AbstractIBatisDao baseDao){
		XlsHandlertongbxx handler = new XlsHandlertongbxx();
		handler.setIn(in);
		handler.setJsonConfig(jsonConfig);
		handler.setEditor(editor);
		handler.setMap(map);//产线map
		handler.setPmap(pmap);//配送类型
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorytongbxx.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,pmap,baseDao);
				//填充错误信息
