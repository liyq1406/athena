 ...
 * @date 2012-7-26
 */
public class XmlSheetHandlerbg extends SheetHandlerbg {
	...
	protected static Logger logger = Logger.getLogger(XmlSheetHandlerbg.class);	//定义日志方法
	public XmlSheetHandlerbg(Document document, String sheet, String table,
	...
	@Override
	public String doSheetHandler(String editor,Map<String,String> cmap,Map<String,String> gyxhdmap,Map<String,String> xhdmap,Map<String,String> xiaohcmap,Map<String,String> xiaohcbhmap,Map<String, String>  lingjmap,AbstractIBatisDao baseDao) {
		try{
			this.editor = editor;
			this.cmap=cmap;
			...
			for (int i = 2; i < list.size(); i++) {
				CkxLingjxhd ljxhd = new CkxLingjxhd();
				...
				for (int j = 0; j < coluNum; j++) {
					if( j == 0)
					{
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z]+$")){
							ljxhd.setUsercenter(fieldValue.get(j).toUpperCase());
						}
					}
					else if( j == 1)
					{
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setLingjbh(fieldValue.get(j).toUpperCase());
						}
					}
					else if (j==2){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setXiaohdbh(fieldValue.get(j).toUpperCase());
						}
					}
					else if (j==3){
						if(null!=fieldValue.get(j)&& !" ".equals(fieldValue.get(j))){
							ljxhd.setShengxr(fieldValue.get(j));
						}
					}
					else if (j==4){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setYuanxhdbh(fieldValue.get(j).toUpperCase());
						}
					}
					else if (j==5){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setWulbh(fieldValue.get(j).toUpperCase());
						}
					}
					//5位
					else if (j==6){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setXiaohcbh(fieldValue.get(j).toUpperCase());
						}
					}
					//4位
					else if (j==7){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setXiaohccxbh(fieldValue.get(j).toUpperCase());
						}
					}
					//2位
					else if (j==8){
						if(null!=fieldValue.get(j)&& fieldValue.get(j).matches("^[A-Za-z0-9_]+$")){
							ljxhd.setCeng(fieldValue.get(j).toUpperCase());
						}
					}
			   }
			    datas.add(ljxhd);
			...
			for (int i=0;i<datas.size();i++) {
				...
				String s="";
				//校验用户中心
				if(null==xhd.getUsercenter() || StringUtils.isBlank(String.valueOf(xhd.getUsercenter())) || xhd.getUsercenter().length()!=2){
					if(null==xhd.getUsercenter()){
						xhd.setUsercenter("");
					}
					result = resultMessage(index,"用户中心："+xhd.getUsercenter()+" 必填并且长度必须为2位");
				...
				if(null==xhd.getLingjbh() || StringUtils.isBlank(xhd.getLingjbh()) || xhd.getLingjbh().length()!=10){
					...
					result = resultMessage(index,"零件编号："+xhd.getLingjbh()+" 必填并且长度必须为10位");
					wronginfo=wronginfo.append(result);
				}else{
					//校验零件编号的存在有效性
					 if (null==lingjmap.get(xhd.getUsercenter()+xhd.getLingjbh())){
						 result = resultMessage(index,"零件编号："+xhd.getLingjbh()+" 不存在或已失效");
						wronginfo=wronginfo.append(result);
					 }
				}
				...
				if(null==xhd.getXiaohdbh() || StringUtils.isBlank(xhd.getXiaohdbh()) || xhd.getXiaohdbh().length()!=9){
					...
					if(!xhd.getXiaohdbh().equals(gyxhdmap.get(xhd.getXiaohdbh()))){
						...
						//增加消耗点编号对应的分配区 在内部物流中存在且有效的校验
						String fenpq = xhd.getXiaohdbh().substring(0,5);//取消耗点的前5为对应分配区
						CkxInnerPathAndModle InnerPath = new CkxInnerPathAndModle();
						InnerPath.setUsercenter(xhd.getUsercenter());
						InnerPath.setLingjbh(xhd.getLingjbh());
						...
						if(Innerlist.size()==0){
							result = resultMessage(index,"消耗点编号："+xhd.getXiaohdbh()+" 分配区："+fenpq+" 在内部物流中不存在");
							wronginfo=wronginfo.append(result);
						}
					}
				}
				...
				//原消耗点编号  必须要提前存在 
				if(null==xhd.getYuanxhdbh() || StringUtils.isBlank(xhd.getYuanxhdbh()) || xhd.getYuanxhdbh().length()!=9){
					if(null==xhd.getYuanxhdbh()){
						xhd.setYuanxhdbh("");
					}
					result = resultMessage(index,"原消耗点编号："+xhd.getYuanxhdbh()+" 必填 只能由数字、字母以及下划线组成 并且长度必须为9位");
					wronginfo=wronginfo.append(result);
				}else{
					//因为带零件校验,所以必须保证 零件不为空,否则不校验
					if(null!=xhd.getLingjbh() && !StringUtils.isBlank(xhd.getLingjbh())){
						//校验原消耗点编号的存在且有效性
						if(null==xhdmap.get(xhd.getUsercenter()+xhd.getLingjbh()+xhd.getYuanxhdbh())){
							result = resultMessage(index,"用户中心："+xhd.getUsercenter()+"零件编号："+xhd.getLingjbh()+" 对应消耗点编号："+xhd.getYuanxhdbh()+" 不存在或已失效");
							wronginfo=wronginfo.append(result);
						}else{
							//原消耗点编号存在校验成功后,根据原消耗点 来取生产线  看分配区对应的是否存在
							//String f=xhd.getYuanxhdbh().substring(0,5);//取消耗点的前5为对应分配区
							String f=xhd.getXiaohdbh().substring(0,5);//0011629 hanwu 20150817  取现消耗点的前5为对应分配区，而非原消耗点
	//						s =this.cmap.get(xhd.getUsercenter()+f);//根据用户中心+分配区编号，取产线
				...
				//增加校验 消耗点和原消耗点必须对应的产线一致
				if((null!=xhd.getYuanxhdbh() && !StringUtils.isBlank(xhd.getYuanxhdbh())) && (null!=xhd.getXiaohdbh() && !StringUtils.isBlank(xhd.getXiaohdbh()))){
					if(null!=s && !s.equals("")){
						String yfenpq=xhd.getYuanxhdbh().substring(0,5);//取消耗点的前5为对应分配区
						String ys = this.cmap.get(xhd.getUsercenter()+yfenpq);
						if(!ys.equals(s)){
							result = resultMessage(index,"消耗点编号"+xhd.getXiaohdbh()+" 与原消耗点编号"+xhd.getYuanxhdbh()+" 在同一用户中心："+xhd.getUsercenter()+" 下对应的产线不一致");
							wronginfo=wronginfo.append(result);
						}
					}
				}
				//0011368 增加校验 消耗点 和 原消耗点 不能一样
				if((null!=xhd.getYuanxhdbh() && !StringUtils.isBlank(xhd.getYuanxhdbh())) && (null!=xhd.getXiaohdbh() && !StringUtils.isBlank(xhd.getXiaohdbh()))){
					 if(xhd.getYuanxhdbh().equals(xhd.getXiaohdbh())){
						 	result = resultMessage(index,"消耗点编号"+xhd.getXiaohdbh()+" 与原消耗点编号"+xhd.getYuanxhdbh()+" 不能一样");
							wronginfo=wronginfo.append(result);
					 }
				}
				//校验物流描述编号  选填
				if(null!=xhd.getWulbh()){
					if(xhd.getWulbh().length()>4 ){
						result = resultMessage(index,"物流描述编号："+xhd.getWulbh()+" 长度最大为4位");
						wronginfo=wronginfo.append(result);
					}
				}
				// 非 必填  校验小火车编号 用户中心+产线+小火车号
				if(null!=s && !"".equals(s) && null!=xhd.getXiaohcbh() && !StringUtils.isBlank(xhd.getXiaohcbh())){
					if(xhd.getXiaohcbh().length()>5){
						result = resultMessage(index,"小火车编号："+xhd.getXiaohcbh()+" 长度最大为5位");
						wronginfo=wronginfo.append(result);
					}else{
						//校验存在性
				...
				if(null!=s && !"".equals(s) && null!=xhd.getXiaohccxbh() && !StringUtils.isBlank(xhd.getXiaohccxbh())){
					if(xhd.getXiaohccxbh().length()>4){
						result = resultMessage(index,"小火车车厢编号："+xhd.getXiaohccxbh()+" 长度最大为4位");
						wronginfo=wronginfo.append(result);
					}else{
						//校验存在性
				...
				if(null!=xhd.getCeng() && !StringUtils.isBlank(xhd.getCeng())){
					if(xhd.getCeng().length()>2){
						result = resultMessage(index,"层："+xhd.getCeng()+" 长度最大为2位");
						wronginfo=wronginfo.append(result);
					}
				}
		...
		try {
			for (CkxLingjxhd lingjxhd : datasnew) {
				...
				if(DBUtil.checkCount(baseDao,"ts_ckx.getCountLingjxhd", lingjxhd)){
					...
					int num = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("ts_ckx.updatelingxhdbg",lingjxhd);
					//如果更新失败，则表示同时有人在操作该条数据  应该提示给用户
					if(0==num){
						return  "数据中有某一行数据正在被多个用户同时操作，请稍后在更新!";
					}
				}else{
					baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("ts_ckx.insertlingxhdbg",lingjxhd);
				}
				//零件消耗点更新或插入之后,往 变更表中插入或更新一条数据
				try {
					biangLingjxhd(lingjxhd);
				} catch (SQLException e) {
