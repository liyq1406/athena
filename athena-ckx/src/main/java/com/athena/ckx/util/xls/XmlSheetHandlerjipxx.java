 ...
 * @date 2012-7-26
 */
public class XmlSheetHandlerjipxx extends SheetHandlerjipxx {
	private AbstractIBatisDao baseDao;
	private static Properties prop;
	private String editor;
	private Map<String,String> cmap;
	private Map<String,Object> pmap;
	protected static Logger logger = Logger.getLogger(XmlSheetHandlerjipxx.class);	//定义日志方法
	public XmlSheetHandlerjipxx(Document document, String sheet, String table,
			...
			for (int i=0;i<datas.size();i++) {
					...
					if(jipxx.getPeislxbh().length()!=4 || !jipxx.getPeislxbh().matches("^[A-Za-z0-9_]+$")){
						...
						if(null==pmap.get(jipxx.getUsercenter()+jipxx.getPeislxbh())){
							...
							Peislb pb = (Peislb)pmap.get(jipxx.getUsercenter()+jipxx.getPeislxbh());
								if(!"2".equals(pb.getTongbjpbs())){
									result = resultMessage(index,"用户中心："+jipxx.getUsercenter()+"配送类型："+jipxx.getPeislxbh()+"  对应的标识不为 集配");
									wronginfo=wronginfo.append(result);
							     }else{
							    	 jipxx.setPdsbz("2");
							     }
				...
				if(null!=jipxx.getJipbzwz()&& !" ".equals(jipxx.getJipbzwz())) {
					if(jipxx.getJipbzwz().length()!=2 || !jipxx.getJipbzwz().matches("^[A-Za-z0-9_]+$")){
						result = resultMessage(index,"集配包装位置："+jipxx.getJipbzwz()+"  长度必须为2位,只能由数字、字母以及下划线组成");
						wronginfo=wronginfo.append(result);
					}
				}
