 ...
 * @date 2012-5-17
 */
public class XlsHandlerchanxlj {
	private InputStream in;
	private String jsonConfig;
	private String editor;  //导入人
	private Map<String,Fenzx> map;
	private Map<String,Object> pmap;
	private AbstractIBatisDao baseDao;//数据源
	
	public Map<String,Fenzx> getMap() {
		return map;
	}

	public void setMap(Map<String,Fenzx> map) {
		this.map = map;
	}
	...
	private XlsHandlerchanxlj() {
		super();
	}

	public static XlsHandlerchanxlj getInstance(InputStream in,String jsonConfig,String editor,Map<String,Fenzx> map,AbstractIBatisDao baseDao){
		XlsHandlerchanxlj handler = new XlsHandlerchanxlj();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorychanxlj.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
