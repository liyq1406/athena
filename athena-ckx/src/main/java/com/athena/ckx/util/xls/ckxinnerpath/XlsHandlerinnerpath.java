 ...
 * @date 2016-10-21
 */
public class XlsHandlerinnerpath {
	...
	private XlsHandlerinnerpath() {
		super();
	}

	public static XlsHandlerinnerpath getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandlerinnerpath handler = new XlsHandlerinnerpath();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryinnerpath.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
