 ...
 * @date 2012-5-17
 */
public class XlsHandlerouterpath {
	...
	private XlsHandlerouterpath() {
		super();
	}

	public static XlsHandlerouterpath getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandlerouterpath handler = new XlsHandlerouterpath();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryouterpath.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
