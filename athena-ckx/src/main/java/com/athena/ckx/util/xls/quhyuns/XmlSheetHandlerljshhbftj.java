...
public class XmlSheetHandlerljshhbftj extends SheetHandlerljshhbftj {
	...
	public String doSheetHandler(String editor,Map<String,String> cmap,AbstractIBatisDao baseDao) throws BiffException, IOException, ParseException {
		...
		List<QuhYuns> datas = new ArrayList<QuhYuns>();
		...
		ArrayList<QuhYuns> wrongdatas =  new ArrayList<QuhYuns>();
		//用一个新集合接收读取的数据，并且把错误信息放进去
		ArrayList<QuhYuns> datasnew =  new ArrayList<QuhYuns>();
		...
		for (int i = 2; i < list.size(); i++) {
			...
			if(null == fieldValue.get(5)||"".equals(fieldValue.get(5))){
				result = resultMessage(index,"支线取货单价：支线取货单价不能为空");
				wronginfo=wronginfo.append(result);
			}
			
			if(null == fieldValue.get(6)||"".equals(fieldValue.get(6))){
				result = resultMessage(index,"区域集配单价：区域集配单价不能为空");
				wronginfo=wronginfo.append(result);
			}
			
			if(null == fieldValue.get(7)||"".equals(fieldValue.get(7))){
				result = resultMessage(index,"干线运输单价：干线运输单价不能为空");
				wronginfo=wronginfo.append(result);
			}
			
			if(null == fieldValue.get(8)|| "".equals(fieldValue.get(8))){
				result = resultMessage(index,"仓储配送单价：仓储配送单价不能为空");
				wronginfo=wronginfo.append(result);
			}
			
			if(null == fieldValue.get(9)|| "".equals(fieldValue.get(9))){
				result = resultMessage(index,"返空单价：返空单价不能为空");
				wronginfo=wronginfo.append(result);
			}
			if(null != fieldValue.get(0)&& !"".equals(fieldValue.get(0))){
			...
			if(null != fieldValue.get(5)&& !"".equals(fieldValue.get(5))){
				if(!fieldValue.get(5).matches("^[1-9][0-9]{0,9}(?:\\.[0-9]{1,3})?$|0\\.[0-9]{1,3}$|^0$")){
					result = resultMessage(index,"支线取货单价：只能输入0-9999999999.999之间的数字");
					wronginfo=wronginfo.append(result);
			...
			if(null != fieldValue.get(6)&& !"".equals(fieldValue.get(6))){
				if(!fieldValue.get(6).matches("^[1-9][0-9]{0,9}(?:\\.[0-9]{1,3})?$|0\\.[0-9]{1,3}$|^0$")){
					result = resultMessage(index,"区域集配单价：只能输入0-9999999999.999之间的数字");
					wronginfo=wronginfo.append(result);
	
				}
			}
			if(null != fieldValue.get(7)&& !"".equals(fieldValue.get(7))){
				if(!fieldValue.get(7).matches("^[1-9][0-9]{0,9}(?:\\.[0-9]{1,3})?$|0\\.[0-9]{1,3}$|^0$")){
					result = resultMessage(index,"干线运输单价：只能输入0-9999999999.999之间的数字");
					wronginfo=wronginfo.append(result);
	
				}
			}
			if(null != fieldValue.get(8)&& !"".equals(fieldValue.get(8))){
				if(!fieldValue.get(8).matches("^[1-9][0-9]{0,9}(?:\\.[0-9]{1,3})?$|0\\.[0-9]{1,3}$|^0$")){
					result = resultMessage(index,"仓储配送单价：只能输入0-9999999999.999之间的数字");
					wronginfo=wronginfo.append(result);
	
				}
			}
			if(null != fieldValue.get(9)&& !"".equals(fieldValue.get(9))){
				if(!fieldValue.get(9).matches("^[1-9][0-9]{0,9}(?:\\.[0-9]{1,3})?$|0\\.[0-9]{1,3}$|^0$")){
					result = resultMessage(index,"返空单价：返空单价只能输入0-9999999999.999的整数或小数");
					wronginfo=wronginfo.append(result);
			...
			if(wronginfo.toString().equals("")){
				quhYuns.setZhixqhdj(Double.parseDouble(fieldValue.get(5)));
				quhYuns.setQuyjpdj(Double.parseDouble(fieldValue.get(6)));
				quhYuns.setGanxysdj(Double.parseDouble(fieldValue.get(7)));
				quhYuns.setCangcpsdj(Double.parseDouble(fieldValue.get(8)));
				quhYuns.setFankdj(Double.parseDouble(fieldValue.get(9)));	
				double lifmdj=quhYuns.getZhixqhdj()+quhYuns.getQuyjpdj()+quhYuns.getGanxysdj()+quhYuns.getCangcpsdj();
				
				if(lifmdj==0){
					result = resultMessage(index,"支线取货、区域集配、干线运输、仓储配送之和必须大于0");
					wronginfo=wronginfo.append(result);
				}
			}
			...
			b.setUsercenter(quhYuns.getUsercenter());
			b.setGongysdm(quhYuns.getGongysdm());
		//	b.setChengysdm(quhYuns.getChengysdm());  一个用户中心+供应商+生效时间只能有一个承运商。
		...
		//excel与数据库中存在时间重叠
			for (QuhYuns oldbean : oldbeans) {
				if(oldbean!=null ){
					if((oldbean.getShengxsj().compareTo(quhYuns.getShengxsj())<=0 && oldbean.getShixsj().compareTo(quhYuns.getShengxsj())>=0 ) 
					|| (oldbean.getShengxsj().compareTo(quhYuns.getShixsj())<=0   && oldbean.getShixsj().compareTo(quhYuns.getShixsj())>=0 )
					|| (oldbean.getShengxsj().compareTo(quhYuns.getShengxsj())<=0 && oldbean.getShixsj().compareTo(quhYuns.getShixsj())>=0 )
					|| (oldbean.getShengxsj().compareTo(quhYuns.getShengxsj())>=0 && oldbean.getShixsj().compareTo(quhYuns.getShixsj())<=0) 
					){
						if(oldbean.getShengxsj().compareTo(quhYuns.getShengxsj())==0 && oldbean.getShixsj().compareTo(quhYuns.getShixsj())==0){
								continue;
						}else{
...
//			}
	 ...
	 * 插入数据到表
	 */
	public String sqlInsertOrUpdate(ArrayList<QuhYuns> datasnew){
		
		try {
			for (QuhYuns bean : datasnew) {	
				QuhYuns quhYuns= (QuhYuns) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CK).selectObject("ts_quhys.queryQuhYunsByyouxj",bean);	
				if(quhYuns!=null){
					//更新数据
					bean.setBiaos("1");
					bean.setEdit_time(DateTimeUtil.getAllCurrTime());
					bean.setEditor(editor);
					baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("ts_quhys.updateQuhYunsDownload",bean);
				}else{
					bean.setBiaos("1");
					//插入数据
					baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("ts_quhys.insertQuhYuns",bean);		
				}
			}
