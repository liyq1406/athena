 ...
 * @date 2012-5-17
 */
public class SheetHandlerFactoryYssjmb {
	 ...
	 */
	public static SheetHandler getInstance(Document document, String sheet,
		...
		if(clazz==null || "null".equals(clazz)){
			...
			handler = new XmlSheetHandlerYssjmb(document, sheet, table, datasourceId, clazz,keys,dateColumns,dateFormats);
		}else{
...
private static SheetHandler createHandler(Document document, String sheet,
	...
	try {
		...
		sh = (SheetHandler) c.newInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats);
	} catch (Exception e) {
		e.printStackTrace();
	}
