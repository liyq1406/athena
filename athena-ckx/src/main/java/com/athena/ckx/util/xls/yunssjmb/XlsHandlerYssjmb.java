...
public class XlsHandlerYssjmb {
	...
	private XlsHandlerYssjmb() {
		super();
	}

	public static XlsHandlerYssjmb getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandlerYssjmb handler = new XlsHandlerYssjmb();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryYssjmb.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
