	  ...
	  if (EXECUTE_METHODS.contains(method.getName())) {
    	  if (log.isDebugEnabled()) {
    		    ...
    		    log.debug("{pstm-" + id + "} Types: " + getTypeString());
    	      	List l = getValue(); 				//modify by wl 2011-11-24
    	      	String sb = removeBreakingWhitespace(sql);
    	      	try {
    	      		   int i = 0;
    	      		   while(sb.indexOf("?") > -1){
    	      			   sb = sb.replaceFirst("\\?", "'" +l.get(i)+"'");
    	      			   i++;
    	      		   }
    	               log.debug("["+Thread.currentThread().getId()+"]SQL - [ " + sb + " ]");	//modify by wl 2011-11-24
    				} catch (Exception e) {
    				}
    	      }
	  }
