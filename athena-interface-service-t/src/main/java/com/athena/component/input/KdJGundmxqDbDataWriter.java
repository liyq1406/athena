 ...
 * @author GJ
 */
public class KdJGundmxqDbDataWriter extends DbDataWriter {
	...
	private List<String> listDhlj = new ArrayList<String>();
	...
	public boolean beforeRecord(int rowIndex, Object line) {
		...
		String zzlx=line.toString().substring(42, 48).trim();
		String biaos=line.toString().substring(75,76).trim();
		 if((zzlx.equals("97X")||zzlx.equals("97D"))&&"1".equals(biaos)){
			return true;	
		}else{
	    	return false;
	    }
	...
	public boolean before() {
	    ...
	    logger.debug("毛需求版次流水号查询sql：" + sql);
	 ...
	 * @return String 需求版次号 
	 */
	public String queryXuqbcOfmaoxq(Map<String, String> params){
	...
	public void afterRecord(int rowIndex, Record record,Object line) {
		try {
			...
			if(null!=record && record.size() >0){
				//需求日期
				if(!"".equals(record.get("xuqrq").toString())){	
					Map<String,String> params = this.parserParam(rowIndex, record, line);
					//如果来源为CLV，处理CLV数据
					...
					synchronized(this){
						...
						//毛需求表插入版次信息
						this.insertMaoxqmx(record, params.get("xuqrq"));
					}
				}
			}
	...
	public void insertMaoxqmx(Record record,String xuqrq){
		...
		sqlBuf.append("xuqz,xuqrq,lingjbh,xuqsl,danw,zhizlx,xuqsszq,xuqksrq,xuqjsrq,cangkdm,editor,edit_time,creator,create_time,active) ");
		...
		sqlBuf.append(record.getString("xuqz")).append("',to_date('").append(xuqrq).append("','yyyy-MM-dd'),'");
	...
	public void insertMaoxq(String xuqbc,String xuqcfsj,String xuqly){
		...
		sqlBuf.append(xuqly).append("','0','1051',to_date('").append(DateTimeUtil.getAllCurrTime());
		sqlBuf.append("','yyyy-MM-dd HH24:mi:ss'),'1051',to_date('").append(DateTimeUtil.getAllCurrTime()).append("','yyyy-MM-dd HH24:mi:ss'),'1')");
		super.execute(sqlBuf.toString());
	}
	...
	public void saveYicbj(String cuowxxxx,String lingjbh,String usercenter){
		...
		sqlBuf.append(usercenter).append("','200','").append(cuowxxxx).append("','1051','1051','1051',to_timestamp('");
		sqlBuf.append(DateTimeUtil.getAllCurrTime()).append("','yyyy-MM-dd HH24:mi:ss'),'1051',to_timestamp('");
