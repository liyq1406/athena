 ...
 * 
 */
public class QuFenUsercentDataReader extends DbDataReader {
	...
	//订单初始化构造函数
	public QuFenUsercentDataReader(DataParserConfig dataParserConfig) {
	...
	public int readLine() {
		...
		try {
		    ...
		    totalNum = getTotalNum(sql); //总数
		    
			ExchangerConfig[] ecs = dataParserConfig.getWriterConfigs();
			for (ExchangerConfig ec : ecs) {
				outPutTxt(sql,dataFields,flagStr,interfaceCode,ec,totalNum);
			}
     ...
     */
    private void outPutTxt(String sql,DataField[] dataFields,String flagStr,String interfaceCode,ExchangerConfig ec,int totalNum) throws SQLException{
    	outPut = createOut(ec); //创建流 
    	String flagSQL = "false".equals(flagStr)? makeIncrementSql():"";
        //拼接增量的字符串
    	StringBuilder sqlBuffer = new  StringBuilder();
    	sqlBuffer.append(sql);
    	//需要添加用户中心
    	sqlBuffer.append(" AND USERCENTER in ("+getUsercenter(ec.getUsercenter())+") ");
    	sqlBuffer.append(flagSQL);
    	

        List<Map<String,Object>> dataList = GetdoWriterLine(sqlBuffer.toString(),dataFields);
        for(int i=0;i<dataList.size();i++){
			 command = new TxtCommand<Object>(outPut, dataParserConfig, dataList.get(i));
			 command.execute();
	    }
