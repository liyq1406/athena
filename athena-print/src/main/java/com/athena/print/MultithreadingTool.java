...
public class MultithreadingTool {
	...
	public synchronized  void newTaskOperate ( 
		...
		for (int i = 0; i < printQtaskinfo.size(); i++) {
			baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("pcenter.updatePrintQtaskinfo1",printQtaskinfo.get(i));
		}
		
		// 主作业状态为 打印中
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("pcenter.updatePrintQtaskmain3",printQtaskmain.getQid());

		// 打印机辅状态为 繁忙
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("sys.updatePrintDevicestatusBydevice2",printQtaskmain.getSdevicecode());
		...
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).execute("sys.updatePrintDevicestatusBydevice3",bean);
