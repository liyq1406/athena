...
@Component
public class VJWulljService extends BaseService {
	...
	public boolean saveXiaohblzjb(Xiaohblzjb bean) {
		int count = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("hlorder.insertXhblVJ", bean);
		return count > 0;
	}
	...
	public boolean deleteAllZjb(String usercenter) {
		...
		int count = this.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("hlorder.clearZjbVJ",map);
		return count > 0;
	}
	 ...
	 * **/
	public void checkVJXhbl(String dingdlx,String usercenter) {
		// 清理中间表
		this.deleteAllZjb(usercenter);
		// 物流路径关联零件消耗点
		List<Xiaohblzjb> allXhbl = null;
		allXhbl = this.queryXhblVJ(usercenter);
	...
	private List<Xiaohblzjb> queryXhblVJ(String usercenter) {
		Map<String,String>map = new HashMap<String,String>();
		map.put("usercenter", usercenter);
		return this.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("hlorder.queryXhblVJ",map);
	}
