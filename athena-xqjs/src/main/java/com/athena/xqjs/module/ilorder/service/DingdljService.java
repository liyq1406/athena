...
@Component
public class DingdljService extends BaseService {
	@Inject
...
//		map.put("gonghms", bean.getGonghms());
//		CommonFun.mapPrint(map, "订单零件插入查询是否有存在的订单零件参数map");
//		CommonFun.objPrint(bean, "订单零件插入时的订单零件bean");
//		Dingdlj object = this.queryDingljObject(map);
//		if (null == object) {
			int count = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("ilorder.insertDingdlj", bean);
			return count > 0;
//		} else {
//			return false;
//		}
	 ...
	 */
	 ...
	 baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("ilorder.deleteDingdlj",
	 bean);
	 }
	 ...
	 */
	// @Transactional
	// public boolean doUpdate(Dingdlj bean) {
	// int count =
	// baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("ilorder.updateDingdlj",
	// bean);
	// return count > 0;
	// }

	public boolean doUpdateForKd(Object obj) {
		int count = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("ilorder.updateDingdljPart", obj);
		return count > 0;
	}
	...
	public Dingdlj queryDingljObject(Map<String, String> map) {
		return (Dingdlj) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("ilorder.queryAllDingdlj", map);
	}
	 ...
	 */
	// public Map select(Pageable page) {
	// return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectPages("ilorder.queryDingdlj", page);
	// }
	...
	public List<Dingdlj> queryAllDingdlj() {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryAllDingdlj");
	}
	...
	public List<Dingdlj> queryAllDingdlj(Map<String, String> map) {
		...
		CommonFun.logger.debug("根据条件查询订单零件queryAllDingdlj方法的sql语句为：ilorder.queryAllDingdlj");
	...
	public List<Dingdlj> queryAllDingdljForList() {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryAllDingdljlist");
	}
	...
	public List queryAllDingdljForBean(Dingdlj bean) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryAllDingdlj", bean);
	}
	/**
	 * 查询全部，返回map -------未测试
	 * 
	...
	public Map queryAllKDDingdljForMap(Pageable page, Map map) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectPages("ilorder.queryAllKDDingdlj", map, page);
	}
	...
	public  List<Dingdlj>  queryKDDingdljForList(Map map) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryAllKDDingdlj", map);
	}
	/**
	 * 查询全部，返回list -------未测试
	 * 
	...
	public  List<Dingdlj> queryEXPDingdljList(Map map) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryEXPDingdlj", map);
	}
	...
	public  List<Dingdlj> queryEXPKDDingdljList(Map map) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryEXPKDDingdlj", map);
	}
	...
	public Dingd queryDingdByDingdh(Map<String, String> map) {
		...
		bean = (Dingd) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("ilorder.queryDingdByDingdh", map);
		map.clear();
		return bean;
	...
	public List<Dingdlj> queryDingdljListPJS(BigDecimal p0sl, BigDecimal p1sl, BigDecimal p2sl, BigDecimal p3sl,
			String dingdzzsj, String p0fyzqxh, String parrten) {
		String ids = "ilorder.";
		Map map = new HashMap();
		map.put("p0sl", p0sl);
		map.put("p1sl", p1sl);
	 ...
	 * @参数：要货数量，模值
	 */
	public Map<Integer, BigDecimal> getModeValue(BigDecimal yaohl, int mode, int zhous) {
	 ...
	 * @return Map 检索结果
	 */
	public Map<String, Object> queryDingdljByParam(Pageable page, Map<String, String> param) {
		return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectPages("ilorder.queryDingdljByParam", param, page);
	}
	...
	public double queryAllowZzsl(Map<String, String> param) {
		Dingdlj dingdlj = (Dingdlj) baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).selectObject("ilorder.queryAllowGzsl", param);
		// 返回可终止数量
		if (dingdlj == null) {
			return 0;
		}
	 ...
	 * @return 设置值后的map
	 */
	public Map<String, String> getParamsByBean(Map<String, String> map, Dingdlj dingdlj) {
		// 订单号
	 ...
	 * @参数：实体类，获取个数
	 */
	public static Map<Integer, BigDecimal> getPCount(Dingdlj bean, int index) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException  {
	 ...
	 * @参数：实体类，获取个数
	 */
	public static Map<String, BigDecimal> getPCounts(Dingdlj bean, int index) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException  {
		BigDecimal pi = BigDecimal.ZERO;
		CommonFun.logger.debug("获取到pi数量getPCount方法index="+index);
		...
		for (int i = 0; i < index; i++) {
			...
			// method拼接的方法
			Method meth = cls.getMethod(method, new Class[] {});
			//判断meth.invoke()是否为空 如果为空则赋值为0 11859 xh 20151010
			if(null == meth.invoke(bean, null)){
				pi=new BigDecimal(0);
			}else{
				pi = new BigDecimal(meth.invoke(bean, null).toString());
			}
			CommonFun.logger.debug("获取到pi数量getPCount方法pi="+pi);
			map.put(nianZq, pi);
	...
	public Dingdlj addDingdlj(final Map<String, String> param, Anxjscszjb anxcshbxzjb, String loginUser,String mos,String rq,
			String gongysdm)
	...
	public List<Dingdmx> queryForInsertDingdlj(Map<String, String> map) {
		return this.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryForInsertDingdlj", map);
	}
	...
	public void insertList(List all, Object bean) {
		if (!all.isEmpty()) {
			for (Object obj : all) {
				this.baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_XQJS).execute("ilorder.insertDingdlj", obj);
			}
		}
	}
	...
	public List<Dingdlj> queryAllJihyByDingd(Map<String, String> searchMap) {
		...
		{
			return baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CKX).select("ilorder.queryAllJihyByDingd", searchMap);
		}
		return null;
	}
}