...
@Component
public class PplCompareSendAction extends ActionSupport {
	
	private final Log log = LogFactory.getLog(PplCompareSendAction.class);
	@Inject
	...
	public String queryCompareQr(@Param CompareQr bean) {
		...
		setResult( "result", pplCompareSendService.select(bean) );
	...
	public String removeCompareQr(@Param CompareQr bean) { 
		...
		try {
			Message m = new Message(pplCompareSendService.doDelete(bean),"i18n.ckx.xuqjs.i18n_maoxqcxbj");
