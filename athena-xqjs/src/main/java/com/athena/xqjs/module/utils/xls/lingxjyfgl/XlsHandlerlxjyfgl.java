 ...
 * @date 2012-5-17
 */
public class XlsHandlerlxjyfgl {
	...
	private XlsHandlerlxjyfgl() {
		super();
	}

	public static XlsHandlerlxjyfgl getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandlerlxjyfgl handler = new XlsHandlerlxjyfgl();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactorylxjyfgl.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
