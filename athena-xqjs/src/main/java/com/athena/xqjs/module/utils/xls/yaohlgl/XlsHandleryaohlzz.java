 ...
 * @date 2012-5-17
 */
public class XlsHandleryaohlzz {
	...
	private XlsHandleryaohlzz() {
		super();
	}

	public static XlsHandleryaohlzz getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandleryaohlzz handler = new XlsHandleryaohlzz();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryyaohlzz.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
