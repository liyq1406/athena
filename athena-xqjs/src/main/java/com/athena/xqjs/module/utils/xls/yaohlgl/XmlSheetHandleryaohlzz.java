...
public class XmlSheetHandleryaohlzz extends SheetHandleryaohlzz {
	private AbstractIBatisDao baseDao;
	private static Properties prop;
	private String editor;
	private Map<String,String> cmap;
	protected static Logger logger = Logger.getLogger(XmlSheetHandleryaohlzz.class);	//定义日志方法
	public XmlSheetHandleryaohlzz(Document document, String sheet, String table,
	...
	public String doSheetHandler(String editor,Map<String,String> cmap,AbstractIBatisDao baseDao) throws BiffException, IOException, ParseException {
		...
		//导入的数据超过300行 提示不让插入
		if(rowNum>302){
			String  message = "导入数据超过300行,请调整数据行数";
			return message;
		}
		...
		for (int i = 2; i < list.size(); i++) {
			...
			for (int j = 0; j < coluNum; j++) {
				...
				{
					if(null!=fieldValue.get(j)&& !" ".equals(fieldValue.get(j))){
						yhl.setLingjbh(fieldValue.get(j).toUpperCase());
					}
				}
			}
			datas.add(yhl);
		...
		for (int i=0;i<datas.size();i++) {
			...
			if(null==yhl.getLingjbh() || StringUtils.isBlank(yhl.getLingjbh())){
				if(null==yhl.getLingjbh()){
					yhl.setLingjbh("");
				}
				result = resultMessage(index,"零件编号："+yhl.getLingjbh()+" 必填并且长度必须为10位");
				wronginfo=wronginfo.append(result);
			}else if(!yhl.getLingjbh().matches("^[A-Za-z0-9]{10}$")){
				yhl.setLingjbh("");
			...
			index ++;
			yhl.setCreator(editor);
			yhl.setCreate_time(CommonFun.getJavaTime());
			yhl.setEditor(editor);
			yhl.setEdit_time(CommonFun.getJavaTime());
			yhl.setZhuangt("0");
			for(int n=0;n<datasnew.size();n++){
				Yaohl y=datasnew.get(n);
				if(y.getUsercenter().equals(yhl.getUsercenter()) && y.getYaohlh().equals(yhl.getYaohlh()) && y.getCreator().equals(yhl.getCreator())){
		...
		if(wrongdatas.size()<=0){
			...
			//如果数据格式正确则校验要货令号可用性
			StringBuffer yhlh=new StringBuffer("yaohlh in ( ");
			for (int i=0;i<datasnew.size();i++) {
					Yaohl yhl = datas.get(i);
					if(i== datasnew.size()-1){
					yhlh=yhlh.append("'" +yhl.getYaohlh() +"' )");
					}else{
					yhlh=yhlh.append("'" +yhl.getYaohlh() +"' ,");	
					}
			}
		//查询要货令信息
		List<Yaohl> yaohllist=baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CK).select("yhl.queryYaohlxx",yhlh.toString());
		Map<String,String> map=new HashMap<String,String>();
		for (Yaohl yaohl : yaohllist) {
			map.put(yaohl.getUsercenter()+yaohl.getLingjbh()+yaohl.getYaohlh(), yaohl.getYaohlh());
	    }
		//判断要货令是否存在
		index=3;
		for (int i=0;i<datasnew.size();i++) {
			...
			Yaohl yhl = datas.get(i);
			if(null == map.get(yhl.getUsercenter()+yhl.getLingjbh()+yhl.getYaohlh())){
				result = resultMessage(index,"用户中心："+yhl.getUsercenter()+"零件编号："+yhl.getLingjbh()+"要货令号："+yhl.getYaohlh()+"  的数据不存在或不是表达状态");
				wronginfo=wronginfo.append(result);
			}
	...
	@Transactional
	public Integer yaohlzz(ArrayList<Yaohl> datasnew,String editor){
		baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CK).execute("yhl.deleteYaohlzz",editor);
		int count =0;
		for(Yaohl yhl:datasnew){
		  count = baseDao.getSdcDataSource(ConstantDbCode.DATASOURCE_CK).execute("yhl.insertYaohlzz",yhl);
		}
		return count;
	} 
	/**
