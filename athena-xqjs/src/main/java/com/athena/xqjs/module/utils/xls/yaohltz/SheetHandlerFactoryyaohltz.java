 ...
 * @date 2012-5-17
 */
public class SheetHandlerFactoryyaohltz {
	 ...
	 * @param clazz 根据clazz创建出sheet解析器
	 */
	public static SheetHandleryaohltz getInstance(Document document, String sheet,
			String table, String datasourceId, String clazz,String keys,String dateColumns,String dateFormats) {
		SheetHandleryaohltz handler = null;
		if(clazz==null || "null".equals(clazz)){
			//创建默认的sheetHandler解析器处理 
			handler = new XmlSheetHandleryaohltz(document, sheet, table, datasourceId, clazz,keys,dateColumns,dateFormats);
		}else{
			//依据用户的clazz全名 创建用户的sheetHandler对象
...
private static SheetHandleryaohltz createHandler(Document document, String sheet,
	String table, String datasourceId, String clazzStr,String keys,String dateColumns,String dateFormats) {
	SheetHandleryaohltz sh = null;
	try {
		...
		sh = (SheetHandleryaohltz) c.newInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats);
	} catch (Exception e) {
		e.printStackTrace();
	}
