 ...
 * @date 2012-5-17
 */
public class XlsHandleryaohltz {
	...
	private XlsHandleryaohltz() {
		super();
	}

	public static XlsHandleryaohltz getInstance(InputStream in,String jsonConfig,String editor,Map<String,String> map,AbstractIBatisDao baseDao){
		XlsHandleryaohltz handler = new XlsHandleryaohltz();
		handler.setIn(in);
	...
	public String doXlsHandler(){
		...
		try {
			...
			for(int i=0;i<jsonArry.size();i++){
				...
				String error = SheetHandlerFactoryyaohltz.getInstance(document,sheet,table,datasourceId,clazz,keys,dateColumns,dateFormats).doSheetHandler(editor,map,baseDao);
				//填充错误信息
