...
--WL
--2012-05-09
create or replace function xqjs_clvmxq(mxqbanc in varchar2,cf_date in varchar2) return number
       is
...
begin
  --循环滚动毛需求表,查询来源,车间号(线号前三位),线号,用户中心,拆分日期,版次,零件编号,数量,单位,制造路线,标识,需求日期
    for ly in (select laiy,SUBSTR(c_line_code,0,3) as chejh,c_line_code,c_usercenter,t_start,t_end,banc,lingjbc_componenth,n_num,
      c_manage_unit,c_make_line,c_flag,cf_date from in_gundmxq where banc = mxqbanc and cf_date = cf_date) loop
       ...
       EXECUTE IMMEDIATE zyysql into zuoyy; --执行SQL获取作用域值
         if ly.chejh = zuoyy then--如果作用域跟车间号相同,则执行,不同的数据过滤
           cxsql :='select decode(count(mubcx),0,'''|| ly.c_line_code || ''',max(mubcx)) from ckx_chanxhb where usercenter ='''|| ly.c_usercenter ||''' and yuancx = '''|| ly.c_line_code ||'''';
           begin
...
(maoxqmx_seq.nextval,ly.banc,ly.c_usercenter,ly.chejh,chax,zqrow.nianzx,to_date(ly.t_start,'yyyyMMdd'),ly.lingjbc_componenth,ly.n_num,dw,zzxl,zqrow.nianzq,'sys',xqdate,'sys',xqdate,'1',ly.t_start,ly.t_end);
          ...
          exception when no_data_found then --如果报no_data_found异常保存异常报警表
          mxqresult :=xqjs_ycbj(ly.lingjbc_componenth,ly.c_usercenter,'200','目标产线查询没有数据');
          end;
        end if;
       exception when no_data_found then --如果报no_data_found异常保存异常报警表
         mxqresult :=xqjs_ycbj(ly.lingjbc_componenth,ly.c_usercenter,'200','作用域查询没有数据');
       end;
    end loop;
    commit;
    return mxqresult;
