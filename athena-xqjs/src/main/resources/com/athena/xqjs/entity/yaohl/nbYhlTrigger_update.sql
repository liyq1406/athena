create or replace trigger XQJS_NBYHL_UPDATE
  after update on Ck_Yaonbhl
  for each row
declare
...
begin
  --2013-06-27 kong  修改：内部要货令为已交付状态，要货令类别为按需时，不记录 CD模式，并且不记录 MD模式中目的地为消耗点要货数量(孙代银出差)
  if :new.yaohlzt=yhlzt_yjf and :old.yaohlzt!=yhlzt_yjf and :old.yaohllb=yhllb_ax and :new.yaohllx<>'CD' and (:new.yaohllx='MD'and :new.muddlx='1')=false then
         --更新订单明细
     ...
     elsif :new.yaohlzt=yhlzt_yzz and :old.yaohlzt!=yhlzt_yzz and :new.yaohllx<>'CD' and (:new.yaohllx='MD'and :new.muddlx='1')=false then
